...
export function createCrmService({ repository, now = () => new Date() } = {}) {
  ...
  return {
    ...
    async recordQualificationProgress({
      ...
      if (!Number.isInteger(questionCount) || questionCount < 0 || questionCount > 3) {
        throw crmError('El embudo admite como máximo tres preguntas.', 400, 'CRM_QUALIFICATION_INVALID');
      }
      ...
      return repository.updateQualification(conversationId, {
        ...
        selected_path: clean(selectedPath, 120),
