...
export function createCrmService({ repository, now = () => new Date() } = {}) {
  ...
  return {
    async attachLeadAnalysis({
      analysisId, sourceId = null, email = null, phone = null, score, priority,
      verdict, summary, identity, market, climate, recommendation,
      risks = [], nextSteps = [], sources = [], pdfUrl = null,
    } = {}) {
      const normalizedAnalysisId = clean(analysisId, 512);
      const numericScore = Number(score);
      const normalizedPriority = clean(priority, 1)?.toUpperCase();
      if (!normalizedAnalysisId || !Number.isInteger(numericScore) || numericScore < 0 || numericScore > 100
        || !['A', 'B', 'C'].includes(normalizedPriority)) {
        throw crmError('Análisis de lead inválido.', 400, 'CRM_LEAD_ANALYSIS_INVALID');
      }
      const cleanList = (values, maxItems = 12, maxLength = 500) => (
        Array.isArray(values) ? values.slice(0, maxItems).map((value) => clean(value, maxLength)).filter(Boolean) : []
      );
      const cleanSources = Array.isArray(sources) ? sources.slice(0, 20).map((source) => ({
        label: clean(source?.label, 240),
        url: /^https?:\/\//i.test(String(source?.url || '')) ? clean(source.url, 1200) : null,
      })).filter((source) => source.label || source.url) : [];
      let conversation = sourceId ? await repository.findConversationBySourceRef?.(clean(sourceId, 512)) : null;
      if (!conversation) {
        const em = normalizeEmail(email);
        const ph = normalizePhone(phone);
        if (!em && !ph) throw crmError('Falta identificar el contacto analizado.', 400, 'CRM_LEAD_ANALYSIS_CONTACT_REQUIRED');
        const contacts = await repository.findContactsByEmailOrPhone(em, ph);
        const match = matchCrmContact({ email: em, phone: ph, existing: contacts });
        if (match.status !== 'exact') {
          throw crmError('No se pudo identificar una única ficha para el análisis.', 409, 'CRM_LEAD_ANALYSIS_CONTACT_AMBIGUOUS');
        }
        conversation = await repository.findLatestConversationForContact?.(match.contact.id);
      }
      if (!conversation) throw crmError('Conversación no encontrada.', 404, 'CRM_NOT_FOUND');
      const previous = conversation.qualification_data?.lead_analysis;
      if (previous?.analysis_id === normalizedAnalysisId) {
        return { status: 'idempotent', conversation, analysis: previous };
      }
      const analysis = {
        analysis_id: normalizedAnalysisId,
        score: numericScore,
        priority: normalizedPriority,
        verdict: clean(verdict, 2000),
        summary: clean(summary, 4000),
        identity: clean(identity, 4000),
        market: clean(market, 4000),
        climate: clean(climate, 2000),
        recommendation: clean(recommendation, 3000),
        risks: cleanList(risks),
        next_steps: cleanList(nextSteps),
        sources: cleanSources,
        pdf_url: /^https?:\/\//i.test(String(pdfUrl || '')) ? clean(pdfUrl, 1200) : null,
        analyzed_at: now().toISOString(),
      };
      const updated = await repository.updateLeadAnalysis(conversation.id, {
        ...(conversation.qualification_data || {}),
        lead_analysis: analysis,
      });
      return { status: 'accepted', conversation: updated, analysis };
    },
    ...
    async ingestInbound({
      ...
      if (!created) {
        await repository.touchConversation(conversation.id, event.received_at, event.body);
        conversation = await repository.getConversation(conversation.id) || conversation;
      }
...
export function createSupabaseCrmRepository(supabaseAdmin) {
  ...
  return {
    ...
    async findConversationBySourceRef(sourceRef) {
      const { data, error } = await supabaseAdmin.from('crm_conversations').select('*')
        .eq('source_ref', sourceRef).order('updated_at', { ascending: false }).limit(1).maybeSingle();
      if (error) throw crmError('No se pudo localizar la ficha CRM.');
      return data || null;
    },
    async findLatestConversationForContact(contactId) {
      const { data, error } = await supabaseAdmin.from('crm_conversations').select('*')
    ...
    async touchConversation(id, receivedAt, body) {
    ...
    async updateLeadAnalysis(id, qualificationData) {
      const { data, error } = await supabaseAdmin.from('crm_conversations').update({
        qualification_data: qualificationData,
        updated_at: new Date().toISOString(),
      }).eq('id', id).select('*').single();
      if (error) throw crmError('No se pudo guardar el análisis del lead.');
      return data;
    },
    async listConversations(filters = {}) {
      let builder = supabaseAdmin.from('crm_conversations').select('*').order('updated_at', { ascending: false }).limit(200);
    ...
    async updateReplyStatus(id, status, providerMessageId = null) {
      ...
      let result = await supabaseAdmin.from('crm_replies').update(payload).eq('id', id);
      if (result.error && providerMessageId && /colum|column/i.test(String(result.error.message || ''))) {
        result = await supabaseAdmin.from('crm_replies').update({ status }).eq('id', id);
      }
      if (result.error) throw crmError('No se pudo actualizar el estado de la respuesta.');
    },
