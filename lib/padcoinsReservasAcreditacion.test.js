...
describe('padcoinsReservasService — acreditar (mock sede activa/inactiva)', () => {
  it('sin opt-in no acredita al pagador por reserva completada', async () => {
    const { acreditarPadcoinsPorReservaCompletada } = await import('../src/padcoins/padcoinsReservasService.js');
    const result = await acreditarPadcoinsPorReservaCompletada({}, '1', {
      reserva: {
        id: 1,
    ...
    assert.equal(result.omission_reason, 'direct_payer_credit_disabled');
  });
