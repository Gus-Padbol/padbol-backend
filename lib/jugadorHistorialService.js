/**
 * Historial unificado del jugador — adapters de lectura + merge (FASE 1–4).
...
const COMUNIDAD_HISTORIAL_SELECT = [
  'id',
  'autor_user_id',
  'texto',
  'sede_id',
...
/**
 * Contadores batch (sin N+1). Si fallan las tablas, devuelve maps vacíos → null en payload.
 */
async function loadComunidadCountsByPublicacionIds(supabaseAdmin, pubIds) {
  const likes = new Map();
  const comentarios = new Map();
  const ids = [...new Set((pubIds || []).filter((id) => id != null))];
  ...
  for (const chunk of chunkIds(ids)) {
    ...
    if (rErr) {
      if (isMissingTableError(rErr)) break;
      throw rErr;
    }
    for (const row of reac || []) {
      const k = String(row.publicacion_id);
      likes.set(k, (likes.get(k) || 0) + 1);
    }
    ...
    const { data: coms, error: cErr } = await supabaseAdmin
      ...
      .eq('estado', 'activo');
    if (cErr) {
      if (isMissingTableError(cErr)) break;
      throw cErr;
    }
...
export async function fetchComunidadHistorialEvents(supabaseAdmin, user, { sourceLimit }) {
  const userId = user?.id;
  if (!userId) return { events: [], skipped_no_fecha: 0, skipped_estado_desconocido: 0 };

  const { data, error } = await supabaseAdmin
    .from('comunidad_publicaciones')
    .select(COMUNIDAD_HISTORIAL_SELECT)
  ...
  try {
    const counts = await loadComunidadCountsByPublicacionIds(
      supabaseAdmin,
      [...byId.keys()],
    );
    likesMap = counts.likes;
    comentariosMap = counts.comentarios;
  } catch (err) {
  ...
  for (const row of byId.values()) {
    ...
    const r = tryNormalizeComunidadEvent(row, {
      likes_count: countsAvailable ? (likesMap.get(key) || 0) : null,
      comentarios_count: countsAvailable ? (comentariosMap.get(key) || 0) : null,
    });
    if (r.ok) {
      events.push(r.event);
      continue;
    ...
    if (r.reason === 'sin_fecha') skipped_no_fecha += 1;
    else if (r.reason === 'estado_excluido' || r.reason === 'eliminada' || r.reason === 'ocultada') {
      skipped_estado_desconocido += 1;
    }
  }
  return { events, skipped_no_fecha, skipped_estado_desconocido };
}
...
const SOURCE_FETCHERS = {
  ...
  comunidad: fetchComunidadHistorialEvents,
};
