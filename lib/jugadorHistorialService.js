/**
 * Historial unificado del jugador — adapters de lectura + merge (FASE 1 + FASE 2).
...
export async function fetchResultadoHistorialEvents(supabaseAdmin, user, { sourceLimit }) {
  ...
  if (equipoIds.length) {
    ...
    if (error && (error.code === '42703' || /updated_at|ronda|fase|hora/i.test(String(error.message || '')))) {
      const retry = await supabaseAdmin
        .from('partidos')
        .select('id, torneo_id, estado, resultado, equipo_a_id, equipo_b_id, ganador_equipo_id, created_at')
        .eq('estado', 'finalizado')
        .or(orFilter)
        .limit(sourceLimit);
      if (retry.error) {
