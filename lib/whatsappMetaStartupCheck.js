...
const MAX_REQUESTS = 12; // Up to ten WABA pages, one phone read and one app-subscription read.
    ...
    promise = (async () => {
      ...
      const readOnlyFetch = async (input, options = {}) => {
        ...
        const phone = url.pathname === `/${EXPECTED.graphVersion}/${EXPECTED.phoneNumberId}`
          && entries.length === 1
          && url.searchParams.get('fields') === 'id,display_phone_number,is_on_biz_app,platform_type';
        const appSubscriptions = url.pathname === `/${EXPECTED.graphVersion}/${EXPECTED.appId}/subscriptions`
          && entries.length === 0;
        if (!subscriptions && !phone && !appSubscriptions) return rejectRead();
