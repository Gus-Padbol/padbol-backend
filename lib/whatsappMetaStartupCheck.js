...
const MAX_REQUESTS = 11; // Up to ten subscription pages and one sandbox phone read.
...
export function createWhatsappMetaQaStartupCheck({
  ...
  return function runOnce() {
    ...
    promise = (async () => {
      ...
      if (['OUTBOUND_DELIVERY_ENABLED', 'WHATSAPP_CLOUD_SEND_ENABLED', 'BACKGROUND_JOBS_ENABLED', 'PUSH_SEND_ENABLED']
        .some(key => env[key] !== 'false')) return finish({ error: 'META_QA_DELIVERY_MUST_BE_DISABLED' });
      ...
      const readOnlyFetch = async (input, options = {}) => {
        ...
        const phone = url.pathname === `/${EXPECTED.graphVersion}/${EXPECTED.phoneNumberId}`
          && entries.length === 1 && url.searchParams.get('fields') === 'id,display_phone_number';
        if (!subscriptions && !phone) return rejectRead();
      ...
      try {
        ...
        return finish(await Promise.race([verifyImpl({ env, fetchImpl: readOnlyFetch }), timeout]));
