/** Historial unificado del jugador — dominio puro (FASE 1–4). */
...
/** Preview seguro de texto de publicación (code points, no bytes). */
export const HISTORIAL_COMUNIDAD_PREVIEW_MAX = 140;
...
/**
 * Preview plano: sin HTML, sin controles, truncado por code points Unicode.
 */
export function buildComunidadTextoPreview(texto, max = HISTORIAL_COMUNIDAD_PREVIEW_MAX) {
  ...
  const sliced = chars.slice(0, max).join('').trim();
  return sliced || null;
}

export function isComunidadPublicacionVisibleParaAutor(row) {
  const estado = String(row?.estado ?? '').trim().toLowerCase();
  // Solo activas: eliminada/ocultada no se muestran al autor en el modelo actual de Comunidad.
  return estado === 'activa';
}

export function resolveComunidadEditada(row) {
  const created = toIsoOrNull(row?.created_at);
  const updated = toIsoOrNull(row?.updated_at);
  if (!created || !updated) return null;
  const delta = new Date(updated).getTime() - new Date(created).getTime();
 ...
 * @returns {{ ok: true, event } | { ok: false, reason: string }}
 */
export function tryNormalizeComunidadEvent(row, {
  likes_count = null,
  comentarios_count = null,
} = {}) {
  if (row?.id == null) return { ok: false, reason: 'sin_id' };
  if (!isComunidadPublicacionVisibleParaAutor(row)) {
    const estado = String(row?.estado ?? '').trim().toLowerCase();
    if (estado === 'eliminada') return { ok: false, reason: 'eliminada' };
    if (estado === 'ocultada') return { ok: false, reason: 'ocultada' };
    return { ok: false, reason: 'estado_excluido' };
  }

  const occurred_at = toIsoOrNull(row.created_at);
  if (!occurred_at) return { ok: false, reason: 'sin_fecha' };

  const editada = resolveComunidadEditada(row);
  const texto_preview = buildComunidadTextoPreview(row.texto);
  const visPub = row.visibilidad != null && String(row.visibilidad).trim() !== ''
    ? String(row.visibilidad).trim().toLowerCase()
    : null;

  let likes = null;
  if (likes_count != null && Number.isFinite(Number(likes_count))) {
    likes = Math.max(0, Math.floor(Number(likes_count)));
  }
  let comentarios = null;
  if (comentarios_count != null && Number.isFinite(Number(comentarios_count))) {
    comentarios = Math.max(0, Math.floor(Number(comentarios_count)));
  }

  const resumen = editada === true
    ? 'Editaste una publicación'
  ...
  const event = buildHistorialEvent({
    ...
    titulo: 'Publicación',
    resumen,
    referenciaTipo: 'comunidad_publicacion',
    referenciaId: String(row.id),
    payload: {
      publicacion_id: String(row.id),
      texto_preview,
      visibilidad_publicacion: visPub,
