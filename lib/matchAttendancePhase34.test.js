...
import {
  ...
  ensureAttendanceWindowExpiredIfDue,
...
import {
  ATTENDANCE_WINDOW_CLOSED_TITLE,
        ...
        const row = rows.find((candidate) => {
          for (const [key, value] of Object.entries(filters)) {
            if (key.endsWith('__contains')) {
              ...
              const needle = value?.dedupe_key ?? value;
              if (candidate[field]?.dedupe_key !== needle) return false;
              continue;
            }
...
function createPhase34Mock({ partido, participants = [], partidosList = null, notificaciones = [] } = {}) {
  ...
  return {
    ...
    from(table) {
      ...
      if (
        table === 'padbol_match_setup_status'
        || table === 'partidos_abiertos_jugadores'
        || table === 'push_tokens'
        || table === 'jugadores_perfil'
      ) {
        return {
          select() { return this; },
          eq() { return this; },
...
describe('matchAttendance Fase 3.4 — expireAttendanceWindow', () => {
  ...
  it('expiración notifica cierre sin duplicar', async () => {
    process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED = 'true';
    const mock = createPhase34Mock({
      partido: openExpiredPartido(),
      participants: [
        participantRow(CAPTAIN_ID, { attendance_status: MATCH_ATTENDANCE_STATUS.CONFIRMED }),
        participantRow(PLAYER_ID),
        participantRow(OTHER_ID, { attendance_status: MATCH_ATTENDANCE_STATUS.DENIED }),
      ],
    });
    ...
    const deps = {
      ...
      sendPushToUser: async () => ({ ok: true }),
    };

    const first = await expireAttendanceWindow(mock, PARTIDO_ID, {
      now: new Date(NOW),
      deps,
    });
    assert.equal(first.expired, true);
    assert.equal(mock.notificacionesStore.rows.length, 2);
    assert.ok(
      mock.notificacionesStore.rows.every((row) => row.titulo === ATTENDANCE_WINDOW_CLOSED_TITLE),
    );
    assert.equal(
      mock.notificacionesStore.rows.some((row) => row.user_id === OTHER_ID),
      false,
    );

    const second = await expireAttendanceWindow(mock, PARTIDO_ID, {
      now: new Date(NOW),
      deps,
    });
  ...
  it('ensureAttendanceWindowExpiredIfDue cierra ventana vencida (lazy expiry)', async () => {
    process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED = 'true';
    const mock = createPhase34Mock({
      partido: openExpiredPartido(),
      participants: [
    ...
    const result = await ensureAttendanceWindowExpiredIfDue(mock, PARTIDO_ID, {
      now: new Date(NOW),
      deps: {
        tryFinalizeMatchAttendanceRewards: async () => ({ credited: false, rewards: {} }),
        sendPushToUser: async () => ({ ok: true }),
      },
    });
    ...
    assert.equal(result.expired, true);
    assert.equal(mock.partidoStore.row.attendance_collection_status, MATCH_ATTENDANCE_COLLECTION_STATUS.READY);
  });
});

describe('matchAttendance Fase 3.4 — lazy expiry GET jugador', () => {
  const originalFlag = process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED;
  ...
  it('GET jugador refleja expired y cannot_respond_reason tras vencimiento', async () => {
    process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED = 'true';
    const mock = createPhase34Mock({
      partido: openExpiredPartido(),
      participants: [participantRow(PLAYER_ID)],
    ...
    const beforeClose = await getPlayerAttendanceState(mock, PARTIDO_ID, PLAYER_ID, {
      skipLazyExpiry: true,
    });
    assert.equal(beforeClose.player.expired, true);
    assert.equal(beforeClose.player.can_respond, false);
    assert.equal(beforeClose.player.cannot_respond_reason, 'deadline_expired');

    const afterClose = await getPlayerAttendanceState(mock, PARTIDO_ID, PLAYER_ID, {
      deps: {
        tryFinalizeMatchAttendanceRewards: async () => ({ credited: false, rewards: {} }),
        sendPushToUser: async () => ({ ok: true }),
      },
    });
    assert.equal(afterClose.player.expired, true);
    assert.equal(afterClose.player.can_respond, false);
    assert.equal(afterClose.match.collection_status, MATCH_ATTENDANCE_COLLECTION_STATUS.BLOCKED);
    assert.equal(afterClose.player.cannot_respond_reason, 'status_locked');
  });
});
...
describe('matchAttendance Fase 3.4 — fetchExpiredOpenAttendanceWindows', () => {
  ...
  it('sin filtro global — devuelve ventanas vencidas (el skip por sede ocurre en expire)', async () => {
    delete process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED;
