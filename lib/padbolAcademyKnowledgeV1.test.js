...
describe('padbolAcademyKnowledgeV1', () => {
  it('importa módulos academy con versiones esperadas', () => {
    ...
    assert.equal(PADBOL_ACADEMY_KIDS_KNOWLEDGE_VERSION, '1.2.0');
    assert.equal(PADBOL_ACADEMY_RULES_TECHNIQUE_KNOWLEDGE_VERSION, '1.2.0');
  });

  it('formatPadbolAcademyKnowledgeForPrompt compone módulos en orden oficial', () => {
    const text = formatPadbolAcademyKnowledgeForPrompt();
    assert.equal(typeof text, 'string');
    assert.notEqual(text, '');
    assert.match(text, /BASE DE CONOCIMIENTO PADBOL ACADEMY \(v1\.0\.0\)/);
    assert.match(text, /No certifica entrenadores ni árbitros/i);
    assert.match(text, /FIPA/i);

    const rulesIdx = text.indexOf('PADBOL ACADEMY — RULES & TECHNIQUE');
    const playerIdx = text.indexOf('PADBOL ACADEMY — PLAYER');
    const coachIdx = text.indexOf('PADBOL ACADEMY — COACH');
    const kidsIdx = text.indexOf('PADBOL ACADEMY — KIDS');
    assert.ok(rulesIdx > -1 && playerIdx > rulesIdx && coachIdx > playerIdx && kidsIdx > coachIdx);

    const rulesOnly = formatPadbolAcademyKnowledgeForPrompt({ modules: ['rulesTechnique'] });
    assert.match(rulesOnly, /RULES & TECHNIQUE/);
    assert.doesNotMatch(rulesOnly, /PADBOL ACADEMY — KIDS/);
  });

  it('format por módulo vacío solo en aggregate; coach, player, rulesTechnique y kids tienen contenido', () => {
    ...
    assert.notEqual(formatPadbolAcademyKidsKnowledgeForPrompt(), '');
  });

  it('kids incluye contenido formativo seguro esperado', () => {
    const text = formatPadbolAcademyKidsKnowledgeForPrompt();
    assert.match(text, /niños/i);
    assert.match(text, /segura/i);
    assert.match(text, /divertida/i);
    assert.match(text, /coordinación/i);
  ...
  it('conecta academy al prompt productivo de Chivi', () => {
    const chiviContextSource = readFileSync('src/config/chiviContext.js', 'utf8');
    assert.match(chiviContextSource, /knowledge\/academy/);
    assert.match(chiviContextSource, /formatPadbolAcademyKnowledgeForPrompt/);
  });

  it('CHIVI_SYSTEM_PROMPT incluye bloque Padbol Academy esperado', () => {
    assert.match(CHIVI_SYSTEM_PROMPT, /Padbol Academy/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /no certifica/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /servicio/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /cristales/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /niños/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /FIPA/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /BASE DE CONOCIMIENTO PADBOL \(v1\.2\.1\)/);
    assert.match(CHIVI_SYSTEM_PROMPT, /PADBOL ACADEMY — RULES & TECHNIQUE/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /PADBOL ACADEMY — PLAYER/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /PADBOL ACADEMY — COACH/i);
    assert.match(CHIVI_SYSTEM_PROMPT, /PADBOL ACADEMY — KIDS/i);
