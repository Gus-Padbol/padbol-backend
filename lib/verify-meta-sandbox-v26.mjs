...
export async function verifyMetaSandbox({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  ...
  const base = { checkedAt: new Date().toISOString(), kind: 'meta_read_only', graphVersion: EXPECTED.graphVersion,
    ...
    phoneLast4: null, isOnBizApp: null, cloudApiPlatform: null, metaPreflightPassed: false };
  ...
  const phone = await readGraph(EXPECTED.phoneNumberId, { fields: 'id,display_phone_number,is_on_biz_app,platform_type' });
