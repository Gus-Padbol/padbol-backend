...
export const EXPECTED = Object.freeze({
  ...
  wabaId: '928900479733370', phoneNumberId: '1376102838911694',
  credentialRef: 'TEST',
...
export async function verifyMetaSandbox({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  ...
  const base = { checkedAt: new Date().toISOString(), kind: 'meta_read_only', graphVersion: EXPECTED.graphVersion,
    ...
    configuration: config, wabaAppSubscribed: null, sandboxPhoneMatches: null, phoneBelongsToWaba: null,
    webhookCallbackMatches: null, messagesFieldSubscribed: null, webhookSubscriptionActive: null,
    phoneLast4: null, isOnBizApp: null, cloudApiPlatform: null,
    phoneConnected: null, phoneVerified: null, metaPreflightPassed: false };
  ...
  const phone = await readGraph(EXPECTED.phoneNumberId, {
    fields: 'id,display_phone_number,is_on_biz_app,platform_type,status,code_verification_status',
  });
  if (phone.error) return { ...base, ...phone };
  base.sandboxPhoneMatches = String(phone.data.id) === EXPECTED.phoneNumberId;
  const phoneDigits = String(phone.data.display_phone_number ?? '').replace(/\D/g, '');
  base.phoneLast4 = phoneDigits.length >= 4 ? Number(phoneDigits.slice(-4)) : null;
  base.isOnBizApp = phone.data.is_on_biz_app === true;
  base.cloudApiPlatform = phone.data.platform_type === 'CLOUD_API';
  base.phoneConnected = phone.data.status === 'CONNECTED';
  base.phoneVerified = phone.data.code_verification_status === 'VERIFIED';

  const wabaPhones = await readGraph(`${EXPECTED.wabaId}/phone_numbers`, { fields: 'id', limit: '100' });
  if (wabaPhones.error) return { ...base, ...wabaPhones };
  if (!Array.isArray(wabaPhones.data.data)) return { ...base, error: 'META_PHONE_NUMBERS_RESPONSE_INVALID' };
  base.phoneBelongsToWaba = wabaPhones.data.data.some(row => String(row?.id) === EXPECTED.phoneNumberId);
  ...
  base.metaPreflightPassed = base.wabaAppSubscribed && base.sandboxPhoneMatches && base.phoneBelongsToWaba
