...
export const EXPECTED = Object.freeze({
  ...
  wabaId: '1044713588190882', phoneNumberId: '1300908966439481',
  sandboxDigits: '15556711531', credentialRef: 'TEST',
});
...
export async function verifyMetaSandbox({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  ...
  const base = { checkedAt: new Date().toISOString(), kind: 'meta_read_only', graphVersion: EXPECTED.graphVersion,
    ...
    configuration: config, wabaAppSubscribed: null, sandboxPhoneMatches: null, metaPreflightPassed: false };
  ...
  const readGraph = async (path, params = {}) => {
  ...
  base.sandboxPhoneMatches = String(phone.data.id) === EXPECTED.phoneNumberId
    && String(phone.data.display_phone_number ?? '').replace(/\D/g, '') === EXPECTED.sandboxDigits;
