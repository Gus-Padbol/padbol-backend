...
/** AI Gateway — mismo perfil que Chivi (IP + JWT). */
export const aiRateLimit = createRateLimiter({
  name: 'ai',
  windowMsEnv: 'RATE_LIMIT_AI_WINDOW_MS',
  maxEnv: 'RATE_LIMIT_AI_MAX',
  windowMsDefault: 10 * 60 * 1000,
  maxDefault: 15,
  keyGenerator: (req) => `${clientIp(req)}${authKeySuffix(req)}`,
  message: 'Demasiadas consultas de IA. Esperá unos minutos e intentá de nuevo.',
});
...
/** Formularios públicos de licencia — estricto por IP confiable de Express. */
export const licenseRequestsRateLimit = createRateLimiter({
  name: 'license-requests',
  windowMsEnv: 'RATE_LIMIT_LICENSE_REQUESTS_WINDOW_MS',
  maxEnv: 'RATE_LIMIT_LICENSE_REQUESTS_MAX',
  windowMsDefault: 15 * 60 * 1000,
  maxDefault: 8,
  message: 'Demasiadas solicitudes. Esperá unos minutos e intentá de nuevo.',
});

/** Webhook MP — alto para ráfagas reales, acotado ante abuso. */
export const mercadoPagoWebhookRateLimit = createRateLimiter({
  name: 'mercadopago-webhook',
  windowMsEnv: 'RATE_LIMIT_MP_WEBHOOK_WINDOW_MS',
  maxEnv: 'RATE_LIMIT_MP_WEBHOOK_MAX',
  windowMsDefault: 60 * 1000,
  maxDefault: 120,
...
/** POST de control por token de scoreboard (marcación sin JWT). */
export const scoreboardControlWriteRateLimit = createRateLimiter({
  name: 'scoreboard-control-write',
  windowMsEnv: 'RATE_LIMIT_SCOREBOARD_CONTROL_WRITE_WINDOW_MS',
  maxEnv: 'RATE_LIMIT_SCOREBOARD_CONTROL_WRITE_MAX',
  windowMsDefault: 15 * 60 * 1000,
  maxDefault: 300,
  message: 'Demasiadas acciones en el marcador. Esperá unos minutos e intentá de nuevo.',
});

/** GET públicos sensibles — generoso por IP (perfil, rankings, reseñas, scoreboard, etc.). */
export const publicReadRateLimit = createRateLimiter({
  name: 'public-read',
  windowMsEnv: 'RATE_LIMIT_PUBLIC_READ_WINDOW_MS',
  maxEnv: 'RATE_LIMIT_PUBLIC_READ_MAX',
  windowMsDefault: 15 * 60 * 1000,
  maxDefault: 150,
  message: 'Demasiadas solicitudes. Intentá de nuevo en unos minutos.',
});
...
/** Paths GET sujetos a publicReadRateLimit (scraping / enumeración). */
export function matchesPublicReadRateLimitPath(path) {
  const p = normalizeRequestPath(path);
  if (p.startsWith('/api/scoreboard')) return true;
  if (p.startsWith('/api/usuarios/perfil-publico')) return true;
  if (p.startsWith('/api/jugador/perfil-publico')) return true;
  if (p.startsWith('/api/jugadores/perfil-publico')) return true;
  if (p === '/api/rankings' || p.startsWith('/api/rankings/')) return true;
  if (/^\/api\/sedes\/[^/]+\/re(senas|señas)$/i.test(p)) return true;
  if (/^\/api\/sedes\/[^/]+\/partidos$/i.test(p)) return true;
  if (p === '/api/partidos/abiertos') return true;
  if (p === '/api/partidos-abiertos') return true;
  return false;
}

/** Aplica publicReadRateLimit solo a GET en rutas públicas sensibles. */
export function publicReadRateLimitIfMatch(req, res, next) {
  if (req.method !== 'GET') return next();
  const path = normalizeRequestPath(req.originalUrl || req.url || req.path || '');
  if (!matchesPublicReadRateLimitPath(path)) return next();
  return publicReadRateLimit(req, res, next);
}

export function configureRateLimitTrustProxy(app) {
  if (String(process.env.RATE_LIMIT_TRUST_PROXY ?? '').trim().toLowerCase() === 'true') {
