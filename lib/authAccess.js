export function isAdminClubOrSuper(role) {
  return role?.rol === 'super_admin' || (role?.rol === 'admin_club' && Number.isInteger(role.sede_id) && role.sede_id > 0);
}

/** Global commercial/editorial content, deliberately separate from operations admins. */
export function isContentEditorOrSuper(role) {
  return role?.rol === 'super_admin' || role?.rol === 'editor_contenido';
}
...
export async function resolveAuthRoleForUser(user, {
  ...
  // Revocation is authoritative: an email allowlist never grants a role.
...
export async function requireSuperAdminUser(req, res, {
  getAuthenticatedUser,
  fetchUserRoleRowForAuthUser,
  legacySuperAdminEmails = [],
} = {}) {
  ...
  if (role?.rol !== 'super_admin') {
    res.status(403).json({ error: 'Solo super_admin puede acceder a esta operación' });
    return null;
  }
...
export async function requireContentEditorUser(req, res, {
  getAuthenticatedUser,
  fetchUserRoleRowForAuthUser,
  legacySuperAdminEmails = [],
} = {}) {
  ...
  if (!isContentEditorOrSuper(role)) {
    res.status(403).json({ error: 'No tenés permiso para gestionar contenido' });
    return null;
  }
...
async function resolveSedeNombreById(supabaseAdmin, sedeId) {
  ...
  const unique = await supabaseAdmin.from('sedes').select('id', { count: 'exact', head: true }).eq('nombre', nombre);
  if (unique.error) throw unique.error;
  return unique.count === 1 ? nombre : null;
}
...
export async function resolveReservasListScope(role, ownerFilter, supabaseAdmin) {
  ...
  if (role?.rol === 'admin_club') {
    if (!Number.isInteger(role.sede_id) || role.sede_id <= 0) {
...
export function applyReservasListScopeToQuery(query, scope) {
  ...
  if (scope.kind === 'sede') {
    if (scope.sedeNombre) {
      ...
      return query.or(`sede_id.eq.${scope.sedeId},and(sede_id.is.null,sede.eq."${escaped}")`);
    }
