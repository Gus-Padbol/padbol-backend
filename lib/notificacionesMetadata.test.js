...
describe('notificacionesMetadata', () => {
  ...
  it('detecta error PGRST204 de producción por columna data ausente', () => {
    assert.equal(
      isMissingNotificacionesDataColumnError({
        code: 'PGRST204',
        message: "Could not find the 'data' column of 'notificaciones' in the schema cache",
  ...
  it('rechaza PGRST204 por otra columna', () => {
    assert.equal(
      isMissingNotificacionesDataColumnError({
        code: 'PGRST204',
        message: "Could not find the 'titulo' column of 'notificaciones' in the schema cache",
      }),
      false,
    );
  });

  it('rechaza PGRST204 genérico sin referencia a data/notificaciones', () => {
    assert.equal(
      isMissingNotificacionesDataColumnError({
        code: 'PGRST204',
        message: 'Could not find a relationship between public.foo and public.bar in the schema cache',
      }),
      false,
    );
  });

  it('rechaza otro error PostgREST', () => {
    assert.equal(
      isMissingNotificacionesDataColumnError({
        code: 'PGRST301',
        message: "Could not find the 'data' column of 'notificaciones' in the schema cache",
...
describe('notificaciones — schema prod link sin data', () => {
  ...
  it('createNotificacion persiste metadata en link cuando falta data (PGRST204)', async () => {
    const store = buildNotificacionesStoreProdLike();
    const dedupe = buildAttendanceNotificationDedupeKey(PARTIDO_ID, USER_ID, 'initial');
