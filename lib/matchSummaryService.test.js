...
describe('matchSummaryAiParse', () => {
  ...
  it('usa Equipo 1 / Equipo 2 cuando no hay nombre custom de equipo', () => {
    ...
    assert.match(summary.summary, /Equipo 1 (se impuso|venció)/i);
    assert.match(summary.summary, /Equipo 2/i);
    assert.doesNotMatch(summary.summary, /equipo formado por/i);
    assert.doesNotMatch(summary.summary, /@/);
  });

  it('caso real partido 12: fallback sin usernames técnicos ni lenguaje administrativo', () => {
    ...
    assert.match(summary.summary, /6-3/);
    assert.match(summary.summary, /Equipo 1/i);
    assert.match(summary.summary, /Equipo 2/i);
      ...
      deps: {
        ...
        markGenerated: async ({ response }) => {
          ...
          return {
            ...
            prompt_version: 'match-summary@1.5.0',
