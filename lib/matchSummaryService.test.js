...
describe('matchSummaryAiParse', () => {
  ...
  it('buildFallbackMatchSummaryResponse expone metadata interna sin textos técnicos visibles', () => {
    ...
    assert.match(fallback.summary, /Equipo 1 venció a Equipo 2 por 2 sets a 1/i);
    assert.match(fallback.summary, /La Meca Padbol Club|Padbol Center/i);
    assert.match(fallback.summary, /partido cambiante/i);
    assert.match(fallback.summary, /se llevó el primer set 6-4/i);
    assert.match(fallback.summary, /reaccionó en el segundo 6-4/i);
    assert.match(fallback.summary, /cerrarlo 6-3/i);
    assert.match(fallback.summary, /15 de junio de 2026/i);
    assert.ok(fallback.highlights.length >= 2);
    assert.match(fallback.highlights[0].text, /Resultado final: Equipo 1 2-1 Equipo 2/i);
  ...
  it('buildDeterministicMatchSummary genera crónica deportiva para 2-1', () => {
    ...
    assert.match(summary.summary, /Equipo 1 venció a Equipo 2 por 2 sets a 1 en La Meca Padbol Club/i);
    assert.match(summary.summary, /partido cambiante/i);
    assert.match(summary.summary, /reaccionó en el segundo 6-4/i);
    assert.match(summary.summary, /cerrarlo 6-3/i);
  ...
  it('buildDeterministicMatchSummary genera crónica para 2-0', () => {
    ...
    assert.match(summary.summary, /dominó el encuentro/i);
  ...
  it('buildDeterministicMatchSummary usa nombres personalizados del payload', () => {
    ...
    assert.match(summary.summary, /Los Pibes venció a La Meca Team/i);
...
describe('matchSummaryService', () => {
  ...
  it('si IA devuelve JSON inválido, usa fallback y persiste generated', async () => {
    ...
    assert.match(result.summary.summary, /Equipo 1 venció a Equipo 2/i);
    assert.match(result.summary.summary, /6 a 4/i);
  ...
  it('si IA devuelve JSON inválido con sets, usa resumen determinístico útil', async () => {
    ...
    assert.equal(result.storedResponse.metadata.fallback, true);
    assert.equal(result.summary.metadata, undefined);
    assert.match(result.summary.summary, /Equipo 1 venció a Equipo 2 por 2 sets a 1/i);
