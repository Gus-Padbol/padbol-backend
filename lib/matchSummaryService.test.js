...
describe('matchSummaryAiParse', () => {
  ...
  it('buildFallbackMatchSummaryResponse expone metadata.fallback sin textos técnicos visibles', () => {
    const fallback = buildFallbackMatchSummaryResponse({
      payload: setsSamplePayload,
    ...
    assert.match(fallback.summary, /Equipo 1 ganó un partido/i);
    assert.match(fallback.summary, /2 sets a 1/i);
  ...
  it('buildDeterministicMatchSummary genera narrativa útil para sets parejos', () => {
    const summary = buildDeterministicMatchSummary(setsSamplePayload);

    assert.equal(summary.title, 'Resumen del partido');
    assert.match(summary.summary, /muy parejo/i);
    assert.match(summary.summary, /Después de llevarse el primer set 6-4/i);
    assert.match(summary.summary, /perdió el segundo 4-6/i);
    assert.match(summary.summary, /reaccionó en el tercero para cerrarlo 6-3/i);
...
describe('matchSummaryService', () => {
  ...
  it('si IA devuelve JSON inválido con sets, usa resumen determinístico útil', async () => {
    ...
    assert.equal(result.cached, false);
    assert.equal(result.summary.metadata.fallback, true);
    assert.match(result.summary.summary, /Equipo 1 ganó un partido/i);
