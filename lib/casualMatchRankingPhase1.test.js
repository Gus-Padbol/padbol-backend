...
function buildRankingsLeaderboardStore({ statsColumnsAvailable = true } = {}) {
  const rows = [];
...
function createRankingMock({
  ...
  const leaderboardStore = buildRankingsLeaderboardStore({ statsColumnsAvailable });
...
describe('casualMatchRanking — resolveCasualMatchRankingResult', () => {
  ...
  it('rechaza empate manual en puntos agregados', () => {
    const result = resolveCasualMatchRankingResult({
      partido: {
        estado: 'finalizado',
        ganador: 'equipo1',
    ...
    assert.equal(result.reason, 'empate_no_permitido');
  });

  it('rechaza empate manual en sets', () => {
    const result = resolveCasualMatchRankingResult({
      partido: {
        estado: 'finalizado',
        ganador: 'equipo1',
        resultado: { equipo1_sets: 1, equipo2_sets: 1 },
      },
    });
    assert.equal(result.ok, false);
    assert.equal(result.reason, 'empate_no_permitido');
  ...
  it('rechaza scoreboard empatado en sets', () => {
    const result = resolveCasualMatchRankingResult({
      scoreboard: { sets_a: 1, sets_b: 1, estado: 'terminado' },
    });
    assert.equal(result.ok, false);
    assert.equal(result.reason, 'empate_no_permitido');
  });
...
describe('casualMatchRanking — resolveParticipantRankingPoints', () => {
  it('rechaza empate: solo win/loss según ganadorSide', () => {
    const winRp = resolveParticipantRankingPoints({
      participant: { user_id: USER_W1 },
      userSideMap: new Map([[USER_W1, 'equipo1']]),
      mode: 'manual',
      ganadorSide: 'equipo1',
    });
    assert.equal(winRp, CASUAL_RANKING_RP.WIN);

    const lossRp = resolveParticipantRankingPoints({
      participant: { user_id: USER_L1 },
      userSideMap: new Map([[USER_L1, 'equipo2']]),
      mode: 'manual',
      ganadorSide: 'equipo1',
    });
    assert.equal(lossRp, CASUAL_RANKING_RP.LOSS);
  });
