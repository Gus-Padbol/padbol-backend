...
describe('matchRewards Fase 1 — evaluateReservationRewardMode', () => {
  it('reserva sin partido → attendance_only', () => {
    assert.equal(
...
describe('matchRewards — splitMatchPadcoinsPool', () => {
  it('solo organizador elegible recibe reparto como participante (sin bonus batch)', () => {
    ...
    assert.equal(shares[0].kind, 'participant');
  });

  it('varios participantes reparten pool sin bonus organizador por defecto', () => {
    ...
    assert.equal(shares.some((s) => s.kind === 'organizer_bonus'), false);

    const orgShare = shares.find((s) => s.userId === USER_ORG);
    const p1Share = shares.find((s) => s.userId === USER_P1);
    assert.ok(orgShare.amount >= 33);
    assert.ok(p1Share.amount >= 33);
  });

  it('bonus organizador solo cuando includeOrganizerBonus=true (legacy explícito)', () => {
    const shares = splitMatchPadcoinsPool(100, [
      { user_id: USER_ORG, role: MATCH_PARTICIPANT_ROLES.ORGANIZER },
      { user_id: USER_P1, role: MATCH_PARTICIPANT_ROLES.PARTICIPANT },
      { user_id: USER_P2, role: MATCH_PARTICIPANT_ROLES.PARTICIPANT },
    ], USER_ORG, { includeOrganizerBonus: true });
...
describe('matchRewards — processReservationPadcoinsOnComplete', () => {
  it('reserva sin partido no acredita al pagador (attendance_only)', async () => {
    const mode = evaluateReservationRewardMode(
      { id: RESERVA_ID, partido_id: null },
      null,
    );
    assert.equal(mode, RESERVATION_REWARD_MODES.ATTENDANCE_ONLY);

    const result = await processReservationPadcoinsOnComplete(
      { from() { throw new Error('no db'); } },
      { id: RESERVA_ID, user_id: USER_ORG, sede_id: 1, estado: 'completada' },
      null,
    );

    assert.equal(result.mode, RESERVATION_REWARD_MODES.ATTENDANCE_ONLY);
