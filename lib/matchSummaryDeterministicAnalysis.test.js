...
describe('matchSummaryDeterministicAnalysis', () => {
  ...
  it('usa dupla cuando hay dos jugadores confiables y no hay nombre custom', () => {
    const analisis = buildMatchSummaryDeterministicAnalysis({
      ...setsPayload,
      equipos: {
        equipo1: {
    ...
    assert.equal(analisis.ganador.nombre, 'La dupla de Nico Renedo y Gustavo Miguens');
    assert.equal(analisis.ganador.tipo, 'dupla');
    assert.equal(analisis.perdedor.nombre, 'La dupla de Ana López y Bruno');
  });

  it('usa Equipo 1 / Equipo 2 cuando no hay nombre custom de equipo', () => {
    const analisis = buildMatchSummaryDeterministicAnalysis({
      ...setsPayload,
      equipos: {
        equipo1: {
    ...
    assert.equal(analisis.equipos.equipo1.nombre, 'La dupla de Ana y Luis');
    assert.equal(analisis.equipos.equipo2.nombre, 'Equipo 2');
    assert.deepEqual(analisis.equipos.equipo1.jugadores, ['Ana', 'Luis']);
    assert.deepEqual(analisis.equipos.equipo2.jugadores, ['Bruno']);
  });

  it('filtra usernames técnicos de jugadores en analisis_previo', () => {
    const analisis = buildMatchSummaryDeterministicAnalysis({
      ...setsPayload,
      equipos: {
        equipo1: {
    ...
    assert.equal(analisis.ganador.nombre, 'Equipo 1');
    assert.equal(analisis.perdedor.nombre, 'Equipo 2');
    assert.equal(analisis.equipos.equipo1.nombre, 'Equipo 1');
    assert.deepEqual(analisis.equipos.equipo1.jugadores, ['Nico']);
    assert.deepEqual(analisis.equipos.equipo2.jugadores, ['Gus']);
  });
