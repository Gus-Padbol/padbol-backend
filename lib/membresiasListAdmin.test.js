...
describe('listMembresiasAdmin paginación y filtros', () => {
  ...
  it('32–35. no N+1; queries constantes con 1 / 15 / 100', async () => {
    async function measure(n) {
      ...
      // Sin q ni expire: membresias_count + membresias_page + planes_batch + jugadores_batch = 4
      assert.deepEqual(tracker.queries, [
        'membresias_count',
        'membresias_page',
        'planes_batch',
        'jugadores_batch',
      ]);
      assert.equal(tracker.fromCalls.length, 4);
  ...
  it('43–44. errores de count/consulta controlados', async () => {
    ...
    const svcCount = createMembresiasSedeService({ supabaseAdmin: brokenCount });
    await assert.rejects(
      () => svcCount.listMembresiasAdmin(
        { rol: 'super_admin', sede_id: null },
      ...
      from() {
        ...
        return {
          ...
          then(resolve) {
            ...
            return Promise.resolve({
              data: null,
              error: { message: 'page failed', code: 'PGRST000' },
              count: null,
            }).then(resolve);
          },
        };
    ...
    const svcPage = createMembresiasSedeService({ supabaseAdmin: brokenPage });
    await assert.rejects(
      () => svcPage.listMembresiasAdmin(
        { rol: 'super_admin', sede_id: null },
        { sedeId: 10, page: 1, limit: 15 },
      ),
      (e) => Boolean(e && (e.message === 'page failed' || e.code)),
    );
    assert.ok(calls >= 2);
  });
