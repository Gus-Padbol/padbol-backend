...
import {
  ...
  mergeResultadoWithScoreboard,
  normalizeMatchResult,
  normalizeMatchResultFromScoreboard,
  parseScoreboardHistorialSets,
  pickBestScoreboardRow,
} from '../src/partidos/matchSummaryPayload.js';
import { buildFallbackMatchSummaryResponse } from '../src/partidos/matchSummaryAiParse.js';
...
const partido12ScoreboardRow = {
  ...
  score_b: 0,
  historial_sets: [
    { set: 1, games_a: 6, games_b: 4 },
    { set: 2, games_a: 4, games_b: 6 },
    { set: 3, games_a: 6, games_b: 3 },
  ],
  historial_puntos: [],
  cronometro_segundos: 3720,
  cronometro_inicio: null,
  cronometro_pausado: true,
  equipo_a_nombre: 'Equipo 1',
  equipo_b_nombre: 'Equipo 2',
  equipo_a_jugadores: [{ nombre: 'Nico' }],
  equipo_b_jugadores: [{ nombre: 'Gus' }],
  created_at: '2026-06-01T20:00:00.000Z',
  updated_at: '2026-06-01T22:00:00.000Z',
};
...
describe('matchSummaryPayload', () => {
  ...
  describe('scoreboard resultado merge', () => {
    it('parseScoreboardHistorialSets soporta games_a/games_b y a/b', () => {
      const fromGames = parseScoreboardHistorialSets(partido12ScoreboardRow.historial_sets);
      assert.deepEqual(fromGames, [
        { equipo1: 6, equipo2: 4 },
        { equipo1: 4, equipo2: 6 },
        { equipo1: 6, equipo2: 3 },
      ]);

      const fromJsonString = parseScoreboardHistorialSets(JSON.stringify([
        { set: 1, a: 6, b: 4 },
        { set: 2, a: 4, b: 6 },
      ]));
      assert.deepEqual(fromJsonString, [
        { equipo1: 6, equipo2: 4 },
        { equipo1: 4, equipo2: 6 },
      ]);
    });

    it('normalizeMatchResultFromScoreboard construye formato sets con parciales', () => {
      const result = normalizeMatchResultFromScoreboard(partido12ScoreboardRow, 'equipo1');

      assert.equal(result.formato, 'sets');
      assert.equal(result.fuente, 'scoreboard');
      assert.equal(result.marcador_texto, '2-1 (6-4, 4-6, 6-3)');
      assert.deepEqual(result.sets.sets_detalle, [
        { equipo1: 6, equipo2: 4 },
        { equipo1: 4, equipo2: 6 },
        { equipo1: 6, equipo2: 3 },
      ]);
    ...
    it('mergeResultadoWithScoreboard mantiene puntos agregados si historial_sets está vacío', () => {
      const puntos = normalizeMatchResult({ equipo1: 6, equipo2: 4, ganador: 'equipo1' });
      const merged = mergeResultadoWithScoreboard(puntos, {
        ...partido12ScoreboardRow,
        historial_sets: [],
      }, 'equipo1');

      assert.equal(merged.formato, 'puntos_agregados');
      assert.equal(merged.marcador_texto, '6-4');
    });

    it('mergeResultadoWithScoreboard prioriza sets del scoreboard cuando hay historial_sets', () => {
      const puntos = normalizeMatchResult({ equipo1: 6, equipo2: 4, ganador: 'equipo1' });
      const merged = mergeResultadoWithScoreboard(puntos, partido12ScoreboardRow, 'equipo1');

      assert.equal(merged.formato, 'sets');
      assert.equal(merged.fuente, 'scoreboard');
      assert.equal(merged.sets.equipo1_sets, 2);
      assert.equal(merged.sets.equipo2_sets, 1);
    });
  });
  ...
  describe('scoreboard helpers', () => {
    ...
    it('buildScoreboardOpcional incluye resumen de historial y duración', () => {
      ...
      assert.equal(optional.id, 'sb-uuid-1');
      ...
      assert.equal(optional.historial_puntos.length, 1);
    });
  });
  ...
  describe('buildMatchSummaryPayload', () => {
    ...
    it('prioriza sets del scoreboard vinculado sobre puntos agregados de capitanes', async () => {
      const pgPool = createPgPoolMock({
        scoreboardByPartidoRows: [baseScoreboardRow],
      });
      ...
      assert.equal(payload.resultado.formato, 'sets');
      assert.equal(payload.resultado.fuente, 'scoreboard');
      assert.equal(payload.resultado.marcador_texto, '2-1 (6-4, 3-6, 6-2)');
      assert.equal(payload.resultado.ganador, 'equipo1');
      assert.equal(payload.scoreboard_opcional.sets_a, 2);
      assert.equal(payload.analisis_previo.formato, 'sets');
      assert.deepEqual(payload.analisis_previo.parciales, ['6-4', '3-6', '6-2']);
    });
    ...
    it('caso real partido 12: usa historial_sets del scoreboard vinculado', async () => {
      const pgPool = createPgPoolMock({
        partidoRows: [partido12Row],
        scoreboardByPartidoRows: [partido12ScoreboardRow],
        historialPuntosCount: 0,
      });

      const payload = await buildMatchSummaryPayload({
        partidoId: 12,
        userId: partido12Row.capitan_user_id,
        pgPool,
      });

      assert.equal(payload.partido_id, 12);
      assert.equal(payload.resultado.formato, 'sets');
      assert.equal(payload.resultado.fuente, 'scoreboard');
      assert.equal(payload.scoreboard_opcional.id, 'sb-partido-12');
      assert.equal(payload.scoreboard_opcional.cancha, '1');
      assert.equal(payload.scoreboard_opcional.estado, 'terminado');
      assert.deepEqual(payload.scoreboard_opcional.historial_sets, [
        { equipo1: 6, equipo2: 4 },
        { equipo1: 4, equipo2: 6 },
        { equipo1: 6, equipo2: 3 },
      ]);
      ...
      assert.equal(payload.analisis_previo.formato, 'sets');
      assert.equal(payload.analisis_previo.marcador_texto, '2-1');
      assert.equal(payload.analisis_previo.resultado_final_sets.texto, '2-1');
      assert.deepEqual(payload.analisis_previo.parciales, ['6-4', '4-6', '6-3']);
      assert.equal(payload.analisis_previo.parciales_texto, '6-4, 4-6 y 6-3');
      assert.equal(payload.analisis_previo.fue_2_1, true);
      assert.equal(payload.analisis_previo.tercer_set_decisivo, true);
      assert.equal(payload.analisis_previo.frases_sugeridas.definido_en_tercer_set, true);
      assert.equal(payload.analisis_previo.frases_sugeridas.partido_cambiante, true);
      assert.equal(payload.analisis_previo.perdedor_reacciono_segundo_set, true);
      assert.equal(payload.analisis_previo.sets_detalle.length, 3);

      const fallback = buildFallbackMatchSummaryResponse({ payload });
      assert.match(fallback.summary, /6-4/);
      assert.match(fallback.summary, /4-6/);
      assert.match(fallback.summary, /6-3/);
      assert.match(fallback.highlights[1].text, /Parciales: 6-4, 4-6 y 6-3/i);
    });

    it('caso real partido 12: soporta historial_sets como JSON string', async () => {
      const pgPool = createPgPoolMock({
        partidoRows: [partido12Row],
        scoreboardByPartidoRows: [{
          ...partido12ScoreboardRow,
          historial_sets: JSON.stringify(partido12ScoreboardRow.historial_sets),
        }],
      });
