...
test('all four delivery controls must be explicitly false', async () => {
  for (const key of ['OUTBOUND_DELIVERY_ENABLED', 'WHATSAPP_CLOUD_SEND_ENABLED', 'BACKGROUND_JOBS_ENABLED', 'PUSH_SEND_ENABLED']) {
...
test('successful read runs once; log contains only bounded outcomes and no secrets, bodies or URLs', async () => {
  ...
  assert.equal(h.requests.length, 2); assert.equal(h.logs.length, 1);
  for (const { url, options } of h.requests) {
    ...
    assert.deepEqual(options.headers, { Authorization: `Bearer ${env.WHATSAPP_META_TOKEN_TEST}` });
  }
...
test('global staging fetch guard continues blocking Graph while private diagnostic reads exact sandbox', async () => {
  ...
  const target = { fetch: async url => {
    ...
    return url.pathname.endsWith('/subscribed_apps')
      ? response({ data: [{ whatsapp_business_api_data: { id: EXPECTED.appId } }] })
      : response({ id: EXPECTED.phoneNumberId, display_phone_number: EXPECTED.sandboxDigits });
  } };
  ...
  try {
    ...
    assert.equal((await h.runner()).metaPreflightPassed, true); assert.equal(requests, 2);
...
test('subscription absent and sandbox mismatch do not pass', async () => {
  const h = harness({ fetchImpl: async url => url.pathname.endsWith('/subscribed_apps')
    ? response({ data: [{ whatsapp_business_api_data: { id: 'unrelated-app' } }] })
    : response({ id: EXPECTED.phoneNumberId, display_phone_number: '+1 555 000 0000' }) });
  const result = await h.runner();
...
test('untrusted paging next URL is never followed; only the cursor travels to fixed Graph host', async () => {
  ...
  const h = harness({ fetchImpl: async url => {
    ...
    return response({ id: EXPECTED.phoneNumberId, display_phone_number: EXPECTED.sandboxDigits });
  } });
...
test('pagination and request budgets are bounded and never silently mark unknown as unsubscribed', async () => {
  ...
  const over = harness({ verifyImpl: async ({ fetchImpl }) => {
    for (let i = 0; i < 12; i++) await fetchImpl(`https://graph.facebook.com/v26.0/${EXPECTED.wabaId}/subscribed_apps?limit=100`, { method: 'GET', redirect: 'error' });
  } });
  assert.equal((await over.runner()).code, 'META_QA_CHECK_UNAVAILABLE'); assert.equal(over.requests.length, 11);
});
