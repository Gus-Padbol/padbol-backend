...
import { createWhatsappMetaQaStartupCheck, META_QA_RESUBSCRIBE_FLAG, META_QA_STARTUP_FLAG } from './whatsappMetaStartupCheck.js';
  ...
  const runner = createWhatsappMetaQaStartupCheck({ env, logger: line => logs.push(line),
    fetchImpl: async (url, options) => {
      ...
      if (url.pathname.endsWith(`/${EXPECTED.appId}/subscriptions`)) return response({ data: [{ object: 'whatsapp_business_account',
        callback_url: EXPECTED.callbackUrl, active: true, fields: [{ name: 'messages' }] }] });
      if (url.pathname.endsWith('/phone_numbers')) return response({ data: [{ id: EXPECTED.phoneNumberId }] });
...
test('successful read runs once; log contains only bounded outcomes and no secrets, bodies or URLs', async () => {
  ...
  assert.equal(h.requests.length, 4); assert.equal(h.logs.length, 1);
...
test('one-shot repair posts only the fixed QA WABA then verifies it without logging credentials', async () => {
  const requests = [], logs = [];
  const runner = createWhatsappMetaQaStartupCheck({ env: { ...env, [META_QA_RESUBSCRIBE_FLAG]: 'true' },
    logger: line => logs.push(line), fetchImpl: async (url, options) => {
      requests.push({ url: new URL(url), options });
      if (options.method === 'POST') return response({ success: true });
      if (url.pathname.endsWith(`/${EXPECTED.appId}/subscriptions`)) return response({ data: [{ object: 'whatsapp_business_account',
        callback_url: EXPECTED.callbackUrl, active: true, fields: ['messages'] }] });
      if (url.pathname.endsWith('/phone_numbers')) return response({ data: [{ id: EXPECTED.phoneNumberId }] });
      return url.pathname.endsWith('/subscribed_apps')
        ? response({ data: [{ whatsapp_business_api_data: { id: EXPECTED.appId } }] })
        : response({ id: EXPECTED.phoneNumberId, display_phone_number: '+54 11 5555 0468', platform_type: 'CLOUD_API',
          is_on_biz_app: false, status: 'CONNECTED', code_verification_status: 'VERIFIED' });
    } });
  const result = await runner();
  assert.equal(result.code, 'META_QA_PREFLIGHT_PASSED');
  assert.equal(result.activationPerformed, true);
  assert.equal(requests.length, 5);
  assert.equal(requests[0].options.method, 'POST');
  assert.equal(requests[0].url.href, `https://graph.facebook.com/${EXPECTED.graphVersion}/${EXPECTED.wabaId}/subscribed_apps`);
  assert.deepEqual(requests[0].options.headers, { Authorization: `Bearer ${env.WHATSAPP_META_TOKEN_TEST}` });
  assert.equal(logs[0].includes(env.WHATSAPP_META_TOKEN_TEST), false);
});
test('failed one-shot repair stops before verification and emits only bounded provider codes', async () => {
  const h = harness({ env: { ...env, [META_QA_RESUBSCRIBE_FLAG]: 'true' },
    fetchImpl: async () => response({ error: { code: 10, error_subcode: 200, message: env.WHATSAPP_META_TOKEN_TEST } }, 403) });
  const result = await h.runner();
  assert.equal(result.code, 'META_QA_RESUBSCRIBE_REJECTED');
  assert.equal(result.activationPerformed, false);
  assert.equal(result.httpStatus, 403);
  assert.equal(result.providerCode, 10);
  assert.equal(result.providerSubcode, 200);
...
test('global staging fetch guard continues blocking Graph while private diagnostic reads exact sandbox', async () => {
  ...
  try {
    ...
    assert.equal((await h.runner()).metaPreflightPassed, true); assert.equal(requests, 4);
...
test('subscription absent and sandbox mismatch do not pass', async () => {
  const h = harness({ fetchImpl: async url => {
    ...
    if (url.pathname.endsWith('/phone_numbers')) return response({ data: [] });
    return url.pathname.endsWith('/subscribed_apps')
...
test('pagination and request budgets are bounded and never silently mark unknown as unsubscribed', async () => {
  ...
  const over = harness({ verifyImpl: async ({ fetchImpl }) => {
    for (let i = 0; i < 14; i++) await fetchImpl(`https://graph.facebook.com/v26.0/${EXPECTED.wabaId}/subscribed_apps?limit=100`, { method: 'GET', redirect: 'error' });
  } });
  assert.equal((await over.runner()).code, 'META_QA_CHECK_UNAVAILABLE'); assert.equal(over.requests.length, 13);
});
