...
const safeProviderText = (value, maxLength) => typeof value === 'string'
  ...
  return function bindWhatsappQaCrmManualSender({ supabaseAdmin } = {}) {
    ...
    return async function sendWhatsappReply({ conversation, body } = {}) {
      ...
      if (!response.ok || response.redirected || response.url && response.url !== sendUrl) {
        ...
        const providerType = safeProviderText(json?.error?.type, 120);
        const providerMessage = safeProviderText(json?.error?.message, 500);
        const fbtraceId = safeProviderText(json?.error?.fbtrace_id, 200);
