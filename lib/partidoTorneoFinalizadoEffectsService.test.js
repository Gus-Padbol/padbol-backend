...
// An ambiguous scoreboard is not a successful post-result effect, even if advancement succeeded.
it('reports pending effect on scoreboard conflict', async () => {
  const result = await onPartidoTorneoFinalizado({}, { partidoId: 45, fuente: 'manual_admin' }, {
    advanceWinnerIfNeeded: async () => ({ status: 'advanced', destino_partido_id: 47 }),
    ensureScoreboardForCompletedBracketPartido: async () => ({ status: 'conflict', reason: 'scoreboards_duplicados' }),
  });
  assert.equal(result.ok, false);
  assert.equal(result.scoreboard.reason, 'scoreboards_duplicados');
});
