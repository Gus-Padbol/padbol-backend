...
describe('matchSummaryPrompt', () => {
  it('registry resuelve match-summary', () => {
    ...
    assert.equal(formatPromptVersion(prompt), 'match-summary@1.1.0');
    assert.equal(MATCH_SUMMARY_PROMPT_VERSION, 'match-summary@1.1.0');
  });
  ...
  it('prompt menciona scoreboard_opcional', () => {
    ...
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /datos del marcador registrado/i);
  });

  it('prompt permite duración solo si existe', () => {
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /duracion_aproximada_minutos/i);
    assert.match(
      MATCH_SUMMARY_SYSTEM_PROMPT,
      /mencionar duración.*SOLO si duracion_aproximada_minutos existe/is,
    );
    assert.match(
      MATCH_SUMMARY_SYSTEM_PROMPT,
      /Inventar duración si duracion_aproximada_minutos no existe/i,
    );
  });
