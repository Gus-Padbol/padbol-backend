...
describe('matchSummaryPrompt', () => {
  it('registry resuelve match-summary', () => {
    ...
    assert.equal(prompt.version, '1.4.0');
    assert.equal(prompt.system, MATCH_SUMMARY_SYSTEM_PROMPT);
    assert.equal(formatPromptVersion(prompt), 'match-summary@1.4.0');
    assert.equal(MATCH_SUMMARY_PROMPT_VERSION, 'match-summary@1.4.0');
  });
  ...
  it('prompt pide variedad de redacción', () => {
    ...
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /Emails visibles/i);
  });
