...
describe('matchSummaryPrompt', () => {
  it('registry resuelve match-summary', () => {
    ...
    assert.equal(formatPromptVersion(prompt), 'match-summary@1.2.0');
    assert.equal(MATCH_SUMMARY_PROMPT_VERSION, 'match-summary@1.2.0');
  });

  it('prompt pide tono de crónica deportiva', () => {
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /Crónica deportiva/i);
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /máximo 2 párrafos/i);
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /analisis_previo/i);
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /fecha_espanol/i);
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /partido cambiante/i);
  });
