...
describe('matchSummaryPrompt', () => {
  it('registry resuelve match-summary', () => {
    ...
    assert.equal(prompt.version, '1.6.0');
    assert.equal(prompt.system, MATCH_SUMMARY_SYSTEM_PROMPT);
    assert.equal(formatPromptVersion(prompt), 'match-summary@1.6.0');
    assert.equal(MATCH_SUMMARY_PROMPT_VERSION, 'match-summary@1.6.0');
  });
  ...
  it('prompt prohíbe usernames técnicos y lenguaje administrativo en summary', () => {
    ...
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /NO repitas "Equipo 1" ni "Equipo 2"/i);
    assert.match(MATCH_SUMMARY_SYSTEM_PROMPT, /dupla rival/i);
  });
