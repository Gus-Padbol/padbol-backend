...
export function backendRuntime(env = process.env) {
  ...
  const knownMode = ['staging', 'production', 'development', 'test'].includes(mode);
  const staging = mode === 'staging';
  const enabled = (name) => knownMode && env[name] === 'true';
  return Object.freeze({ staging, mode: knownMode ? mode : 'unknown',
...
export function assertStagingIsolation(env = process.env) {
  if (!backendRuntime(env).staging) return;
...
export function installStagingFetchGuard(env = process.env, target = globalThis) {
  ...
  if (!runtime.staging || runtime.outboundDeliveryEnabled) return () => {};
