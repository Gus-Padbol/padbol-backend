...
describe('padcoinsReservasService — límites e idempotencia', () => {
  it('reserva acredita parcial y segunda llamada es ya_acreditada', async () => {
    ...
    const first = await acreditarPadcoinsPorReservaCompletada(supabaseAdmin, '77', {
      allowDirectOrganizerCredit: true,
      reserva,
      reservationConfig: {
...
describe('logrosSyncService — límites en logro desbloqueado', () => {
  ...
  it('logro con sede_id usa override efectivo de sede', async () => {
    const { sumarPadcoinsLogroDesbloqueado } = await import('../src/arena/logrosSyncService.js');
    let insertedMonto = null;
    ...
    const supabaseAdmin = {
      from(table) {
        if (table === 'padcoins_movimientos') {
          return {
            ...
            insert(payload) {
              ...
              return {
                select() { return this; },
                single: async () => ({ data: { ...payload, id: 'mov-logro-sede' }, error: null }),
              };
            },
            then(resolve, reject) {
        ...
        if (table === 'padcoins_saldo') return buildSaldoMock(USER_ID, 0, 0);
        if (table === 'padcoins_global_config') {
          return {
            select() { return this; },
            eq() { return this; },
          ...
          return {
            ...
            maybeSingle: async () => ({
              data: {
                ...
                fecha_inicio: null,
                fecha_fin: null,
                rule_overrides: { logro_desbloqueado: 900 },
              },
              error: null,
            }),
          };
    ...
    assert.ok(result);
    assert.equal(insertedMonto, 900);
    assert.equal(result.monto_aplicado, 900);
  });

  it('logro sin sede_id mantiene monto global', async () => {
    const { sumarPadcoinsLogroDesbloqueado } = await import('../src/arena/logrosSyncService.js');
    let insertedMonto = null;
        ...
        if (table === 'padcoins_movimientos') {
          return {
            ...
            insert(payload) {
              ...
              return {
                select() { return this; },
                single: async () => ({ data: { ...payload, id: 'mov-logro-global' }, error: null }),
              };
            },
            then(resolve, reject) {
              Promise.resolve({ data: [], error: null }).then(resolve, reject);
...
describe('padcoinsEarnLimitsService — límites efectivos por sede', () => {
  function buildSupabaseWithSedeOverrides(ruleOverrides = {}) {
    return {
      from(table) {
        if (table === 'padcoins_movimientos') {
          return buildMovimientosMock([{ monto: 450 }]);
        }
        if (table === 'padcoins_global_config') {
          return {
            select() { return this; },
  ...
  it('getPadcoinsEarnLimitsForSede usa override diario de sede', async () => {
    const limits = await getPadcoinsEarnLimitsForSede(
      buildSupabaseWithSedeOverrides({ limite_diario_jugador: 500 }),
      1,
    );
    assert.equal(limits.limite_diario_jugador, 500);
    assert.equal(limits.limite_mensual_jugador, 10000);
  });

  it('sin override hereda límite global', async () => {
    const limits = await getPadcoinsEarnLimitsForSede(
      buildSupabaseWithSedeOverrides({}),
      1,
    );
    assert.equal(limits.limite_diario_jugador, 1000);
  });

  it('applyPadcoinsEarnCaps con sedeId respeta override diario', async () => {
    const supabaseAdmin = buildSupabaseWithSedeOverrides({ limite_diario_jugador: 500 });
    const result = await applyPadcoinsEarnCaps(supabaseAdmin, USER_ID, 100, {
      now: FIXED_NOW,
      sedeId: 1,
    });
    assert.equal(result.amountToCredit, 50);
    assert.equal(result.capped, true);
    assert.equal(result.reason, 'limite_diario_aplicado');
  });

  it('sin sedeId conserva comportamiento global', async () => {
    const supabaseAdmin = buildSupabaseWithSedeOverrides({ limite_diario_jugador: 500 });
    const result = await applyPadcoinsEarnCaps(supabaseAdmin, USER_ID, 100, {
      now: FIXED_NOW,
      limits: DEFAULT_LIMITS,
