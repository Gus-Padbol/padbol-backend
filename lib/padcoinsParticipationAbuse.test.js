...
import {
  ...
  buildMatchOrganizerBonusSourceKey,
...
describe('padcoinsParticipationAbuse — cierre batch sin bonus organizador', () => {
  it('pagador no participante: cierre batch no crea evento ni movimiento organizer_bonus', async () => {
    const participants = [
      participant(PLAYER_A, { role: MATCH_PARTICIPANT_ROLES.PARTICIPANT }),
    ];
    const supabase = buildPadcoinsSupabase({
      participantsStore: buildParticipantsStore(participants),
    });

    const result = await creditValidatedMatchPadcoins(supabase.admin, {
      matchId: PARTIDO_ID,
      reserva: reservaBase(),
      organizerUserId: PAYER,
      reservationConfig: RESERVATION_CONFIG,
    });

    assert.equal(result.acreditado, true);
    assert.equal(result.credits.length, 1);
    assert.equal(result.credits[0].userId, PLAYER_A);
    assert.equal(
      supabase.rewards.rows.some((row) => row.source_key === buildMatchOrganizerBonusSourceKey(MATCH_TYPES.CASUAL, PARTIDO_ID, PAYER)),
      false,
    );
    assert.equal(
      supabase.movimientos.rows.some((row) => row.user_id === PAYER),
  ...
  it('organizador participante confirmado: cierre batch no duplica ni agrega bonus', async () => {
    const participants = [
      participant(PAYER, { role: MATCH_PARTICIPANT_ROLES.ORGANIZER }),
      participant(PLAYER_A, { attendance_status: MATCH_ATTENDANCE_STATUS.PENDING }),
    ];
    const supabase = buildPadcoinsSupabase({
      participantsStore: buildParticipantsStore(participants),
    ...
    const individual = await creditIndividualAttendancePadcoins(supabase.admin, {
      matchId: PARTIDO_ID,
      userId: PAYER,
      reserva: reservaBase(),
      participants,
    ...
    assert.equal(individual.acreditado, true);

    const batch = await creditValidatedMatchPadcoins(supabase.admin, {
      matchId: PARTIDO_ID,
      reserva: reservaBase(),
      organizerUserId: PAYER,
      reservationConfig: RESERVATION_CONFIG,
    ...
    assert.equal(batch.acreditado, false);
    assert.equal(batch.credits.length, 1);
    assert.equal(batch.credits[0].reason, 'ya_acreditado_event');
    assert.equal(
      supabase.rewards.rows.filter((row) => row.user_id === PAYER && row.status === MATCH_REWARD_EVENT_STATUS.CREDITED).length,
      1,
    );
    assert.equal(
      supabase.movimientos.rows.filter((row) => row.user_id === PAYER && row.tipo === PADCOINS_MOVEMENT_TYPES.EARN).length,
      1,
    );
  });

  it('varios participantes confirmados: un evento por jugador, sin bonus organizer', async () => {
    const participants = [
    ...
    assert.equal(batch.acreditado, true);
    assert.equal(batch.credits.filter((row) => row.acreditado === true).length, 2);
    assert.equal(supabase.rewards.rows.length, 2);
    assert.equal(
      supabase.rewards.rows.every((row) => String(row.source_key).startsWith('attendance|match|')),
      true,
    );
    assert.equal(
      supabase.rewards.rows.some((row) => String(row.source_key).includes('organizer_bonus')),
      false,
    );
  });

  it('reejecución del cierre batch no duplica eventos ni movimientos', async () => {
    const participants = [participant(PLAYER_A)];
    const supabase = buildPadcoinsSupabase({
      participantsStore: buildParticipantsStore(participants),
    });

    const first = await creditValidatedMatchPadcoins(supabase.admin, {
      matchId: PARTIDO_ID,
      reserva: reservaBase(),
      organizerUserId: PAYER,
      reservationConfig: RESERVATION_CONFIG,
    });
    const second = await creditValidatedMatchPadcoins(supabase.admin, {
      matchId: PARTIDO_ID,
      reserva: reservaBase(),
      organizerUserId: PAYER,
      reservationConfig: RESERVATION_CONFIG,
    ...
    assert.equal(supabase.rewards.rows.length, 1);
    assert.equal(supabase.movimientos.rows.filter((row) => row.tipo === PADCOINS_MOVEMENT_TYPES.EARN).length, 1);
  });

  it('cancelación revierte solo recompensas reales del batch', async () => {
    const participants = [participant(PLAYER_A)];
    const rewardStore = buildRewardEventsStore();
    const movStore = buildMovimientosStore();
    const supabase = buildPadcoinsSupabase({
    ...
    await creditValidatedMatchPadcoins(supabase.admin, {
      matchId: PARTIDO_ID,
      reserva: reservaBase(),
      organizerUserId: PAYER,
      reservationConfig: RESERVATION_CONFIG,
    ...
    const reversal = await revertirPadcoinsParticipacionPorReserva(supabase.admin, RESERVA_ID, {
      reserva: reservaBase({ estado: 'cancelada' }),
    });

    assert.equal(reversal.revertido, true);
    assert.equal(reversal.participation_reversals.filter((row) => row.revertido === true).length, 1);
    assert.equal(
      rewardStore.rows.filter((row) => row.status === MATCH_REWARD_EVENT_STATUS.REVERSED).length,
      1,
    );
    assert.equal(movStore.rows.filter((row) => row.tipo === PADCOINS_MOVEMENT_TYPES.REVERSE).length, 1);
  });
});
