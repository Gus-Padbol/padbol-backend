...
import {
  ...
  resolveAttendanceReminderStages,
...
describe('matchAttendance Fase 3.5 — dedupe y textos', () => {
  ...
  it('resolveAttendanceReminderStages acumula etapas vencidas', () => {
    const opened = new Date('2026-07-08T12:00:00.000Z');
    assert.deepEqual(
      resolveAttendanceReminderStages(opened, new Date('2026-07-09T13:00:00.000Z')),
      ['reminder_24h'],
    );
    assert.deepEqual(
      resolveAttendanceReminderStages(opened, new Date('2026-07-10T13:00:00.000Z')),
      ['reminder_24h', 'reminder_48h'],
    );
  });
});
...
describe('matchAttendance Fase 3.5 — recordatorios', () => {
  ...
  it('recordatorio 48h enviado una sola vez', async () => {
    process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED = 'true';
    process.env.MATCH_ATTENDANCE_REMINDERS_ENABLED = 'true';
    const mock = createPhase35Mock({
      partido: {
    ...
    const titles = mock.notificacionesStore.rows.map((row) => row.titulo);
    assert.ok(titles.includes(ATTENDANCE_REMINDER_24H_TITLE));
    assert.ok(titles.includes(ATTENDANCE_REMINDER_48H_TITLE));

    const second = await processAttendanceReminders(mock, { now, batchSize: 20 });
    assert.equal(second.sent, 0);
    assert.equal(second.duplicates, 2);
  });

  it('confirmado y denied no reciben recordatorios', async () => {
    process.env.MATCH_ATTENDANCE_CONFIRMATION_ENABLED = 'true';
    process.env.MATCH_ATTENDANCE_REMINDERS_ENABLED = 'true';
    const mock = createPhase35Mock({
      partido: {
      ...
      participants: [
        pendingParticipant(CAPTAIN_ID, { attendance_status: MATCH_ATTENDANCE_STATUS.CONFIRMED }),
        pendingParticipant(PLAYER_ID, { attendance_status: MATCH_ATTENDANCE_STATUS.DENIED }),
      ],
    });
