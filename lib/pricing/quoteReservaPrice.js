...
// Las nuevas reservas no agregan comisión de Padbol Match al jugador.
...
export function assertClientPrecioMatchesQuote(clientPrecio, serverTotal, tolerance = PRICE_TOLERANCE_ABS) {
  ...
  // Los clientes instalados pueden seguir mostrando la cotización anterior.
  // Se reconoce ese total calculado por servidor; nunca se usa para cobrar.
  const legacyClientTotal = serverTotal + Math.round(serverTotal * 0.03);
  if (Math.abs(client - serverTotal) > tolerance
      && Math.abs(client - legacyClientTotal) > tolerance) {
    throw Object.assign(
