...
describe('casualMatchRanking Fase 2 — stats delta helpers', () => {
  it('computeStatsDeltaForOutcome win/loss', () => {
    ...
    assert.equal(computeStatsDeltaForOutcome('draw'), null);
    assert.equal(computeStatsDeltaForOutcome('empate'), null);
  });
...
describe('casualMatchRanking Fase 2 — empate manual rechazado', () => {
  it('no acredita RP ni incrementa empatados', async () => {
    const participants = [
      {
        user_id: USER_W,
        attendance_status: MATCH_ATTENDANCE_STATUS.ADMIN_VALIDATED,
    ...
    assert.equal(leaderboardStore.rows.length, 0);
  });
});
