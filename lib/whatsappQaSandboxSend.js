    ...
    return Object.freeze({
      async handleWebhook(payload) {
        ...
        const sender = {
          async sendText({ channel, toWaId, body, replyToProviderMessageId }) {
            ...
            if (!claimed || !exactChannel(channel) || body !== QA.reply
              || !Number.isFinite(expiresAt) || now().getTime() >= expiresAt) fail();
            ...
            try {
              const request = (async () => {
                const response = await fetchImpl(SEND_URL, {
                  ...
                  body: JSON.stringify(buildWhatsappTextPayload({ toWaId, body: QA.reply, replyToProviderMessageId })),
                });
                if (response.redirected || response.status >= 300 && response.status < 400
                  || response.url && response.url !== SEND_URL || !response.ok) fail('WHATSAPP_QA_SANDBOX_SEND_UNCERTAIN');
        ...
        const service = createWhatsappCloudService({ repository: scopedRepository, sender,
          sendEnabled: true, maxSendAttempts: 1, now });
