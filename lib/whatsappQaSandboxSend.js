...
// This diagnostic never overrides the environment-wide outbound kill switch.
// When general outbound delivery is disabled, no sender factory exists.
export function createWhatsappQaSandboxServiceFactory({
  ...
  if (env.OUTBOUND_DELIVERY_ENABLED !== 'true') return null;
  if (env.BACKEND_RUNTIME_MODE !== 'staging' || env.STAGING_SUPABASE_PROJECT_REF !== QA.project
    ...
    || ['BACKGROUND_JOBS_ENABLED', 'PUSH_SEND_ENABLED']
