...
const QA = Object.freeze({
  ...
  phone: '1376102838911694', waba: '1044713588190882',
...
const boundedDiagnosticText = (value, maxLength) => typeof value === 'string'
...
function createSendDiagnostic(logger) {
  return event => {
    ...
    const safe = {
      ...
      providerType: boundedDiagnosticText(event.providerType, 120),
      providerMessage: boundedDiagnosticText(event.providerMessage, 500),
      fbtraceId: boundedDiagnosticText(event.fbtraceId, 200),
      recipient,
    };
              ...
              const request = (async () => {
                ...
                if (!response.ok) {
                  ...
                  if (!controller.signal.aborted) diagnostic({ phase: 'provider_rejection', httpStatus: response.status,
                    ...
                    providerType: rejected?.error?.type, providerMessage: rejected?.error?.message,
                    fbtraceId: rejected?.error?.fbtrace_id });
                  fail('WHATSAPP_QA_SANDBOX_SEND_UNCERTAIN');
                }
