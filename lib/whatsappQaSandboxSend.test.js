...
const channel = () => ({ id: '81bba438-eb65-4f70-bdf9-32f6c113d5bf',
  tenant_id: '3a2e5306-e1fc-4b24-b03b-fe965fc37e97', meta_phone_number_id: '1376102838911694',
  meta_waba_id: '928900479733370', credential_ref: 'TEST', auto_reply_text: REPLY, active: true,
...
const environment = () => ({
  ...
  WHATSAPP_CLOUD_MAX_SEND_ATTEMPTS: '1', OUTBOUND_DELIVERY_ENABLED: 'true',
...
const payload = (overrides = {}) => ({ object: 'whatsapp_business_account', entry: [{ id: '928900479733370',
  changes: [{ field: 'messages', value: { metadata: { phone_number_id: '1376102838911694' },
...
test('sandbox opt-in is exact and rejects every incompatible runtime or gate before I/O', () => {
  ...
  const invalid = {
    ...
    BACKGROUND_JOBS_ENABLED: 'true', PUSH_SEND_ENABLED: 'true',
  ...
  assert.equal(createWhatsappQaSandboxServiceFactory({
    env: { ...environment(), OUTBOUND_DELIVERY_ENABLED: 'false' },
  }), null);
});
...
test('provider rejection diagnostics keep the complete safe Meta error without secrets', async () => {
  const diagnostics = [];
  const db = database({
    meta_phone_number_id: '1376102838911694',
    meta_waba_id: '928900479733370',
  });
  const env = environment();
  env.OUTBOUND_DELIVERY_ENABLED = 'false';
  const factory = createWhatsappQaSandboxServiceFactory({
    env,
    now: () => NOW,
    ...
    fetchImpl: async () => ({
      ...
      status: 403,
      redirected: false,
      url: 'https://graph.facebook.com/v26.0/1376102838911694/messages',
      json: async () => ({ error: {
        message: 'Meta rejected the request\nfor this asset',
        type: 'OAuthException',
        code: 200,
        error_subcode: 2018001,
        fbtrace_id: 'A-safe-trace-id',
        access_token: 'must-never-be-logged',
      } }),
    }),
  });
  const body = payload();
  body.entry[0].id = '928900479733370';
  body.entry[0].changes[0].value.metadata.phone_number_id = '1376102838911694';
  await factory({ supabaseAdmin: db }).handleWebhook(body);
  const rejection = diagnostics.find(item => item.phase === 'provider_rejection');
  assert.deepEqual(rejection, {
    phase: 'provider_rejection',
    httpStatus: 403,
    providerCode: 200,
    providerSubcode: 2018001,
    providerType: 'OAuthException',
    providerMessage: 'Meta rejected the request for this asset',
    fbtraceId: 'A-safe-trace-id',
    recipient: null,
  });
  assert(!JSON.stringify(diagnostics).includes('must-never-be-logged'));
  assert(!JSON.stringify(diagnostics).includes(env.WHATSAPP_META_TOKEN_TEST));
});
...
test('global outbound kill switch prevents creation of the sandbox sender', async () => {
  const env = { ...environment(), OUTBOUND_DELIVERY_ENABLED: 'false' };
  const calls = [];
...
test('actual release mount reports sandbox disabled while all global gates are false', async () => {
  ...
  try {
    ...
    assert.equal(ready.body.whatsappQaSandboxSendEnabled, false);
