import { getTournamentCompletionEvidence } from './tournamentCompletionService.js';
...
const PARTIDO_SELECT = 'id,torneo_id,estado,grupo,equipo_a_id,equipo_b_id,ganador_equipo_id,resultado,bracket_round,bracket_position,partido_siguiente_id,partido_siguiente_slot';
...
export async function getTorneosResumenStats(supabaseAdmin, { role, query } = {}, options = {}) {
  ...
  // Automatic sporting closure grants no ranking points. Recover its champion
  // from the verified linked final using the rows already loaded in this batch.
  // Existing podium rows/legacy formats remain authoritative and unchanged.
  for (const torneo of scopedTorneos) {
    const tid = Number(torneo.id);
    if (String(torneo.estado).toLowerCase() !== 'finalizado' || winnerByTorneo.has(tid)) continue;
    const matches = partidosByTorneo.get(tid) || [];
    if (!matches.some(p => p.bracket_round != null)) continue;
    const teams = equiposByTorneo.get(tid) || [];
    const closure = getTournamentCompletionEvidence(torneo, matches, teams);
    if (!closure.verified || closure.final_partido_id == null) continue;
    const final = matches.find(p => Number(p.id) === Number(closure.final_partido_id));
    if (Number(final?.ganador_equipo_id) !== Number(closure.ganador_equipo_id)) continue;
    const team = teams.find(e => Number(e.id) === Number(closure.ganador_equipo_id));
    if (team) winnerByTorneo.set(tid, { equipoId: team.id, nombre: team.nombre ?? null });
  }
