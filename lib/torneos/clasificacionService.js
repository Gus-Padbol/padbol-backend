...
export function partidoIsKnockoutStage(partido, tipoTorneo) {
  ...
  if (!partido.grupo && Number.isInteger(partido.bracket_round) && partido.bracket_round > 0) return true;
...
function findFinalPartido(partidos) {
  ...
  const linked = finished.filter(p => !p.grupo && Number.isInteger(p.bracket_round) && p.bracket_round > 0);
  if (linked.length) {
    const lastRound = Math.max(...linked.map(p => p.bracket_round));
    const finals = linked.filter(p => p.bracket_round === lastRound && p.partido_siguiente_id == null);
...
export function buildKnockoutRankingRows(partidos, equipos = []) {
  ...
  const semiPartidos = finished
    .filter((p) => {
      ...
      return r === 'semifinal' || r === 'semifinales'
        || (finalP?.bracket_round > 1 && !p.grupo && p.bracket_round === finalP.bracket_round - 1);
    })
  ...
  const cuartosPartidos = finished
    .filter((p) => normalizeRonda(p.ronda) === 'cuartos'
      || (finalP?.bracket_round > 2 && !p.grupo && p.bracket_round === finalP.bracket_round - 2))
    .sort((a, b) => Number(a.id) - Number(b.id));
