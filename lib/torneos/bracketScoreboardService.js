...
export async function ensureScoreboardForCompletedBracketPartido(
  ...
  const activeRows = (existentes ?? []).filter(sb => !ESTADOS_SCOREBOARD_TERMINADO.has(normalizeEstado(sb.estado)));
  if (activeRows.length > 1) return buildResult({ status:'conflict', reason:'scoreboards_duplicados', partidoId:id });
  const activo = activeRows[0];
  if (activo) {
    return buildResult({
      status: 'skipped',
  ...
  if (errInsert?.code === '23505') {
    // A competing worker may have won the database's unique active-scoreboard constraint.
    // Reuse only one valid row; never choose arbitrarily among duplicates or rotate its token.
    const { data: rows, error } = await supabaseAdmin.from('scoreboard_partidos')
      .select('id, estado, torneo_id, sede_id').eq('partido_torneo_id',id);
    const active = (rows ?? []).filter(row => !ESTADOS_SCOREBOARD_TERMINADO.has(normalizeEstado(row.estado)));
    if (!error && active.length === 1 && Number(active[0].torneo_id) === Number(partido.torneo_id)
      && Number(active[0].sede_id) === Number(partido.sede_id ?? torneo?.sede_id)) {
      return buildResult({status:'skipped',reason:'scoreboard_existente',partidoId:id,scoreboardId:active[0].id});
    }
    return buildResult({status:'conflict',reason:'scoreboard_conflicto',partidoId:id});
  }
  if (errInsert) {
