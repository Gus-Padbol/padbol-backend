...
export async function onPartidoTorneoFinalizado(supabaseAdmin, params, deps = {}) {
  ...
  try {
    ...
    if (['failed','conflict'].includes(advanceResult?.status)) ok = false;
    const nextSlotConfirmed = advanceResult?.status === 'advanced'
      || (advanceResult?.status === 'skipped' && advanceResult?.reason === 'ya_avanzado');
    // Retry also repairs a scoreboard that failed after the winning team had advanced.
    if (nextSlotConfirmed && advanceResult.destino_partido_id != null) {
      try {
        scoreboardResult = await ensureScoreboardFn(supabaseAdmin, {
          partidoId: advanceResult.destino_partido_id,
        });
        if (['failed','conflict'].includes(scoreboardResult?.status)) ok = false;
      } catch (scoreboardErr) {
        ok = false;
        scoreboardResult = {
          status: 'failed',
