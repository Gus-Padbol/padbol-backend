...
/**
 * Normaliza la fila persistida de un torneo (o su ausencia) al scope
 * que deben usar las rutas admin: existencia + sede_id real de la base.
 * Nunca usa un sede_id enviado por el cliente.
 *
 * @param {{ sede_id?: number|string|null } | null | undefined} row
 * @returns {{ exists: boolean, sedeId: number|null }}
 */
export function resolveTorneoRowScope(row) {
  if (!row) return { exists: false, sedeId: null };
  return { exists: true, sedeId: normalizeTorneoSedeId(row.sede_id) };
}
