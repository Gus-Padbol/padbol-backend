...
export async function advanceWinnerIfNeeded(supabaseAdmin, { partidoId }) {
  ...
  if (slotValue == null) {
    if (!['pendiente','programado'].includes(normalizeEstado(destino.estado))) {
      return buildResult({ status: 'conflict', reason: 'destino_protegido', partidoId: id,
        destinoPartidoId: destino.id, slot: destinoSlot.slot, ganadorEquipoId });
    }
    ...
    const { data: updated, error: errUpdate } = await supabaseAdmin
      .from('partidos').update(patch).eq('id', destino.id).eq('torneo_id', partido.torneo_id)
      .eq('estado', destino.estado).is(destinoSlot.column, null).select('id').maybeSingle();
    ...
    if (!updated) {
      const { data: current, error } = await supabaseAdmin.from('partidos')
        .select(PARTIDO_DESTINO_SELECT).eq('id', destino.id).maybeSingle();
      const same = !error && current && Number(current.torneo_id) === Number(partido.torneo_id)
        && Number(current[destinoSlot.column]) === Number(ganadorEquipoId);
      return buildResult({ status: same ? 'skipped' : 'conflict',
        reason: same ? 'ya_avanzado' : 'destino_cambio', partidoId: id,
        destinoPartidoId: destino.id, slot: destinoSlot.slot, ganadorEquipoId });
    }
    return buildResult({
      status: 'advanced',
