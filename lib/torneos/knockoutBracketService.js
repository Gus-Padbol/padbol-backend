...
export function buildKnockoutBracketMatches({
  ...
  const roundLabels = KNOCKOUT_ROUND_LABELS[equipos.length];
  const numRounds = roundLabels.length;
  const partidos = [];

  for (let roundIdx = 0; roundIdx < numRounds; roundIdx += 1) {
    const bracketRound = roundIdx + 1;
    const ronda = roundLabels[roundIdx];
    const isFinal = roundIdx === numRounds - 1;
    ...
    for (let position = 1; position <= matchesInRound; position += 1) {
      ...
      const row = {
        ...
        estado: 'pendiente',
        grupo: null,
        fase: isFinal ? 'final' : 'eliminatoria',
        ronda,
