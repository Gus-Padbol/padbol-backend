...
function buildSedeConfigMock(active = true) {
  return {
    select() { return this; },
    eq() { return this; },
    maybeSingle: async () => ({
      data: active
        ? { id: 'cfg-1', sede_id: 1, activo: true, fecha_inicio: null, fecha_fin: null }
        : null,
      error: null,
    }),
  };
...
function buildSupabaseForPenalty({
  ...
  return {
    from(table) {
      ...
      if (table === 'padcoins_sede_config') return buildSedeConfigMock(sedeActive);
