...
function memoryCrmRepository() {
  ...
  return {
    ...
    async findConversationBySourceRef(sourceRef) {
      return conversations.find((c) => c.source_ref === sourceRef) || null;
    },
    async findLatestConversationForContact(contactId) {
      return [...conversations].reverse().find((c) => c.contact_id === contactId) || null;
    },
    ...
    async touchConversation(id, receivedAt, body) {
      ...
      if (row) { row.received_at = receivedAt; row.updated_at = receivedAt; row.inbound_body = body; }
    },
    ...
    async updateLeadAnalysis(id, qualificationData) {
      const row = conversations.find((c) => c.id === id);
      if (!row) return null;
      row.qualification_data = qualificationData;
      return row;
    },
...
test('el análisis del lead vuelve a la misma ficha y es idempotente', async () => {
  const repo = memoryCrmRepository();
  const svc = createCrmService({ repository: repo, now: () => new Date('2026-09-24T18:00:00Z') });
  await svc.ingestInbound({
    source: 'form', sourceId: 'request-123', channel: 'email',
    email: 'club@example.com', nombre: 'Ana Pérez', origin: 'web_form:dev_padbol_contacto_business',
  });
  const payload = {
    analysisId: 'analysis-request-123-v1', sourceId: 'request-123', score: 84, priority: 'A',
    verdict: 'Lead de alto potencial.', summary: 'Club existente con proyecto concreto.',
    market: 'Mercado local favorable.', climate: 'Conviene evaluar una solución indoor.',
    recommendation: 'Coordinar una reunión.', risks: ['Validar fondos'],
    nextSteps: ['Solicitar plano'], sources: [{ label: 'Sitio institucional', url: 'https://example.com' }],
    pdfUrl: 'https://files.example.com/report.pdf',
  };
  const first = await svc.attachLeadAnalysis(payload);
  const replay = await svc.attachLeadAnalysis(payload);

  assert.equal(first.status, 'accepted');
  assert.equal(replay.status, 'idempotent');
  assert.equal(repo.conversations.length, 1);
  assert.equal(repo.conversations[0].qualification_data.lead_analysis.score, 84);
  assert.equal(repo.conversations[0].qualification_data.lead_analysis.priority, 'A');
  assert.equal(repo.conversations[0].qualification_data.lead_analysis.pdf_url, payload.pdfUrl);
});
...
test('WhatsApp agrupa mensajes posteriores del mismo contacto y origen en el hilo abierto', async () => {
  ...
  assert.equal(repo.conversations[0].inbound_body, 'Necesito información');
  assert.equal(second.conversation.inbound_body, 'Necesito información');
});
