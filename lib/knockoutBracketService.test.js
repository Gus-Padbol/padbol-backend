...
describe('getKnockoutRoundLabel', () => {
  it('calcula labels sin persistirlos en DB', () => {
    assert.equal(getKnockoutRoundLabel({ totalTeams: 4, bracketRound: 1 }), 'semifinal');
    assert.equal(getKnockoutRoundLabel({ totalTeams: 4, bracketRound: 2 }), 'final');
    assert.equal(getKnockoutRoundLabel({ totalTeams: 8, bracketRound: 1 }), 'cuartos');
    assert.equal(getKnockoutRoundLabel({ totalTeams: 16, bracketRound: 1 }), 'octavos');
    assert.equal(getKnockoutRoundLabel({ totalTeams: 8, bracketRound: 9 }), null);
  });
});
...
describe('buildKnockoutBracketMatches', () => {
  it('4 equipos crea 3 partidos: 2 semifinales y 1 final', () => {
    ...
    assert.deepEqual(rows.map((r) => r.ronda), [1, 1, 2]);
  ...
  it('8 equipos crea 7 partidos', () => {
    ...
    assert.equal(rows.filter((r) => r.ronda === 1).length, 4);
    assert.equal(rows.filter((r) => r.ronda === 2).length, 2);
    assert.equal(rows.filter((r) => r.ronda === 3).length, 1);
  });
  ...
  it('16 equipos crea 15 partidos', () => {
    ...
    assert.equal(rows.filter((r) => r.ronda === 1).length, 8);
    assert.equal(rows.filter((r) => r.ronda === 2).length, 4);
    assert.equal(rows.filter((r) => r.ronda === 3).length, 2);
    assert.equal(rows.filter((r) => r.ronda === 4).length, 1);
  });
  ...
  it('primera ronda con equipos reales y rondas futuras null', () => {
    ...
    const final = rows.find((r) => r.bracket_round === 2);
    assert.equal(final.equipo_a_id, null);
    assert.equal(final.equipo_b_id, null);
  });

  it('no genera keys fase ni es_final y usa ronda integer', () => {
    const rows = buildKnockoutBracketMatches({
      equipos: equipos(8),
      torneoId: 50,
      sedeId: 7,
    ...
    const final = rows.find((r) => r.bracket_round === 3);

    assert.ok(rows.every((r) => typeof r.ronda === 'number'));
    assert.ok(rows.every((r) => !Object.hasOwn(r, 'fase')));
    assert.ok(rows.every((r) => !Object.hasOwn(r, 'es_final')));
    assert.equal(final.ronda, 3);
...
describe('linkBracketMatches', () => {
  it('4 equipos: semis apuntan a final slots A/B y final sin link saliente', () => {
    ...
    const final = merged.find((p) => p.bracket_round === 2);
