...
/** Diagnóstico seguro del destinatario: nunca expone el número completo. */
export function describeWhatsappRecipient(raw) {
  const value = String(raw ?? '');
  const digitCount = value.replace(/\D/g, '').length;
  const last4 = value.slice(-4);
  const masked = value.length > 4
    ? `${'•'.repeat(value.length - 4)}${last4}`
    : '•'.repeat(value.length);
  return {
    digitCount,
    last4,
...
export function createWhatsappCloudService({
  ...
  async function attemptOutbox(outbox) {
    ...
    const reportPhase = phase => {
      try { Promise.resolve(onSendDiagnostic?.({ phase })).catch(() => {}); } catch { /* Diagnostics never change delivery state. */ }
    };
    try {
      reportPhase('before_transport');
      ...
      if (acknowledgementReceived) reportPhase('persistence_failed');
  ...
  return {
    async handleWebhook(payload) {
      ...
      for (const inbound of incoming) {
        ...
        // Invoke on replay too: its CRM side is idempotent, and this lets a retry
        // recover if WhatsApp persisted before the CRM transaction completed.
        const guided = await onInbound?.({ channel, inbound, inboundRow: stored.row });
        ...
        const outbox = await repository.ensureOutbox({
          channel,
          inboundRow: stored.row,
          body: cleanText(guided?.response, MAX_MESSAGE_TEXT_LENGTH)
        ...
        const delivery = await attemptOutbox(outbox.row);
        results.push({
          created: stored.created,
          outboxCreated: outbox.created,
...
export function registerWhatsappCloudRoutes(app, {
  ...
  app.post(WHATSAPP_CLOUD_WEBHOOK_PATH, async (req, res) => {
    logger?.info?.('[whatsapp-cloud] webhook received', {
      code: 'WHATSAPP_WEBHOOK_RECEIVED',
      hasRawBody: Buffer.isBuffer(req.rawBody),
      hasSignature: typeof req.headers['x-hub-signature-256'] === 'string',
    });
    ...
    try {
      ...
      logger?.info?.('[whatsapp-cloud] webhook processed', {
        code: 'WHATSAPP_WEBHOOK_PROCESSED',
        received: Number.isInteger(result.received) ? result.received : 0,
      });
