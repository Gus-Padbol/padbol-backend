    ...
    from(table) {
      const api = {
        ...
        update(patch) {
          const filters=[];
          const write={
            eq:(key,value)=>{filters.push(row=>row[key]===value);return write;},
            is:(key,value)=>{filters.push(row=>row[key]==null&&value==null);return write;},
            select:()=>write,
            maybeSingle:()=>{
              const current=state.destino;
              if(!current || !filters.every(fn=>fn(current)))return Promise.resolve({data:null,error:null});
              onDestinoUpdate?.(patch,current.id);
              state.destino={...current,...patch};
...
describe('advanceWinnerIfNeeded', () => {
  ...
  it('no agrega un equipo a destino finalizado', async () => {
    const { supabaseAdmin, state } = createAdvanceMock({
      fuente: partidoFuente({ partido_siguiente_slot: 'B' }),
      destino: partidoDestino({
        equipo_a_id: 3,
    ...
    assert.equal(result.status, 'conflict');
    assert.equal(state.destino.equipo_b_id, null);
    assert.equal(state.destino.estado, 'finalizado');
  });

  it('no agrega un equipo a destino en_curso', async () => {
    const { supabaseAdmin, state } = createAdvanceMock({
      fuente: partidoFuente({ partido_siguiente_slot: 'B' }),
      destino: partidoDestino({
        equipo_a_id: 3,
