...
/** Expone campos opcionales del partido solo si vienen en la fila de DB. */
function pickOptionalPartidoFields(row) {
  if (!row || typeof row !== 'object') return {};

  const keys = [
    'define_campeon',
    'consagra_campeon',
    'es_partido_consagracion',
    'resultado_define_campeon',
    'impacto_tabla',
    'achievement',
    'logro',
    'fase',
    ...
    'defineCampeon',
    'consagraCampeon',
    'esPartidoConsagracion',
    'resultadoDefineCampeon',
    'impactoTabla',
    'nombreFase',
    'esFinal',
    'is_final',
...
export function mapTorneoPublicRow(row) {
  ...
  return {
    ...
    formato_equipo: row.formato_equipo ?? 'dobles',
  };
}
...
export function mapPartidoTorneoPublicRow(row) {
  ...
  const scoreboardId = row.scoreboard_id ?? row.scoreboard?.id ?? null;
  ...
  return {
    ...
    scoreboard_estado: row.scoreboard_estado ?? null,
    marcador_disponible: Boolean(scoreboardId),
    cancha: row.cancha ?? null,
    ganador_equipo_id: row.ganador_equipo_id ?? null,
