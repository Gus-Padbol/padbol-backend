...
/** Compat pre-migración (sin visibilidad). */
export const EQUIPO_USUARIO_SELECT_LEGACY = [
  'id',
  'nombre',
  'deporte',
  'capitan_user_id',
...
export const EQUIPO_JUGADOR_SELECT_LEGACY = [
  'id',
  'equipo_id',
  'user_id',
  'email',
...
export function isMissingEquipoColumnError(error, column) {
  const msg = String(error?.message ?? '').toLowerCase();
  const col = String(column ?? '').toLowerCase();
  return (
    error?.code === '42703'
    || error?.code === 'PGRST204'
    || (msg.includes(col) && (msg.includes('column') || msg.includes('does not exist')))
  );
}
