...
- Four added profile regressions cover declared sports, the absent optional legacy sports table, an existing table and an empty table (eight focused profile tests passed). Six independent HTTP cases passed against the actual PostgreSQL 17 QA schema over verified TLS: UUID/profile ID/username return the public DTO, while email/missing UUID/injection text return 404. The same transaction passed all 16 privacy checks and was rolled back with no fixture rows remaining.
...
Direct modification of authoritative fields within one's profile (for example federation/XP) is a separate authorization issue; closing anonymous disclosure does not claim to solve it. Two inherited prize policies also trust the self-editable profile's sede_id instead of an administrative role; this authorization bypass must be closed before production. Administrative pending-profile and level-validation UI has no equivalent mounted server API yet and cannot keep updating other profiles directly after migration 11. Real push delivery, public WhatsApp verification, PDF publication/downloads and user-facing flows require coordinated QA before production.
